...
func (s *Server) fetchServerID(addr string) (string, float64) {
	start := time.Now()
	client := &http.Client{Timeout: 3 * time.Second}
	resp, err := client.Get("http://" + addr + "/server_info")
	if err != nil {
		return "", 0
	}
	defer resp.Body.Close()
	...
	id, _ := result["server_id"].(string)
	if id == "" {
		return "", latency
	}
	sig, _ := result["server_id_signature"].(string)
	if sig == "" {
		return "", latency
	}
	pubKeyB64, _ := result["public_key"].(string)
	if pubKeyB64 == "" {
		return "", latency
	}
	...
	if !VerifyRawTextSignature(id, sig, string(pubKeyPEM)) {
		return "", latency
	}
	return id, latency
}
