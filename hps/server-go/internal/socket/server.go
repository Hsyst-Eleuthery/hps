...
func (s *Server) fetchContentFromNetwork(contentHash string) bool {
	...
	for _, serverAddr := range servers {
		if time.Now().After(deadline) {
			log.Printf("content propagation: deadline exceeded hash=%s", contentHash)
			break
		}
		select {
		case ok := <-clientResult:
...
func (s *Server) resolveDNSFromNetwork(domain string) bool {
	...
	for _, serverAddr := range servers {
		addr := serverAddr
		go func() {
			okDNS, dnsData, _ := s.server.MakeRemoteRequestJSON(addr, "/dns/"+url.PathEscape(domain), http.MethodGet, nil)
...
processResult:
	...
	if ddnsHash == "" || !fileExists(ddnsPath) {
		okDDNS, ddnsBytes, _ := s.server.MakeRemoteRequestBytes(lastAddr, "/ddns/"+url.PathEscape(domain), http.MethodGet)
