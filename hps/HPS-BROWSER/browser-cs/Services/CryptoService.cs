...
public sealed class CryptoService
{
    ...
    public byte[] DeriveLocalStorageKey(string username, string passphrase)
    {
        ...
        try
        {
            ...
            try
            {
                var salt = SHA256.HashData(Encoding.UTF8.GetBytes(normalized + ":hps-browser-db-salt"))[..SaltSizeBytes];
                var derived = Rfc2898DeriveBytes.Pbkdf2(localPrivateBytes, salt, 600000, HashAlgorithmName.SHA256, AesKeySizeBytes);
                var result = new byte[salt.Length + derived.Length];
                Buffer.BlockCopy(salt, 0, result, 0, salt.Length);
                Buffer.BlockCopy(derived, 0, result, salt.Length, derived.Length);
