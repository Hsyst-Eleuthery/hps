...
public sealed class HpsSocketService
{
    ...
    public async Task ConnectAsync(string serverUrl, CancellationToken cancellationToken = default, bool isReconnect = false)
    {
        ...
        // C2: TLS certificate validation
        _socket.Options.RemoteCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) =>
        {
            if (sslPolicyErrors == SslPolicyErrors.None) return true;
            // Allow connections to servers with valid certificates or self-signed certs on local network
            // In production, pin specific certificates or use certificate transparency checks
            return false;
        };
