...
func (s *Server) registerHandlers() {
	...
	s.io.OnEvent("/", "cancel_dns", s.handleCancelDNS)
	s.io.OnEvent("/", "cancel_content", s.handleCancelContent)
	s.io.OnEvent("/", "cancel_flow", s.handleCancelFlow)
...
func (s *Server) handleAuthenticate(conn socketio.Conn, data map[string]any) {
	...
	emitFlowProgress(conn, "login", "user_lookup", 2, 3, "Consultando registro de usuário...", 3000)
...
func (s *Server) handleTransferHPS(conn socketio.Conn, data map[string]any) {
	...
	if assignedMiner != "" {
		s.emitToUser(assignedMiner, "miner_signature_request", map[string]any{
			...
			"deadline_sec": int(nowTs+5.0) - int(nowTs),
		})
	...
	changeNote := ""
	if feeAmount > 0 {
		feeNote = fmt.Sprintf(" Taxa: %d HPS.", feeAmount)
	}
	if adjustedAmount != amount {
		changeNote = fmt.Sprintf(" Troco: %d HPS.", amount-adjustedAmount)
	}
	emitFlowProgress(conn, "transfer", "finalizing", 5, 5, "Finalizando transferência...", 2000)
	conn.Emit("hps_transfer_ack", map[string]any{
		"success":     true,
...
func (s *Server) handleSignTransfer(conn socketio.Conn, data map[string]any) {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("PANIC in notifyMonetaryTransferUpdate goroutine transfer=%s err=%v", transferID, r)
			}
		}()
		s.notifyMonetaryTransferUpdate(transferID, "signature_submitted", "", nil)
	}()
}
...
func (s *Server) runPendingSignatureActionWorker(transferID string) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC in runPendingSignatureActionWorker transfer=%s err=%v", transferID, r)
		}
		s.signatureWorkerMu.Lock()
...
func (s *Server) runPendingSignatureActionWorkerPayload(transferID, actionID string, payload map[string]any) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC in runPendingSignatureActionWorkerPayload transfer=%s err=%v", transferID, r)
		}
		s.signatureWorkerMu.Lock()
		delete(s.signatureWorkers, transferID)
		s.signatureWorkerMu.Unlock()
...
func (s *Server) rollbackExchangeTransfer(transfer map[string]any, reason string) {
	...
	go func(issuerAddress, tokenID, transferID, owner string, totalValue int, reason string) {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("PANIC in rollbackExchangeTransfer goroutine issuer=%s transfer=%s err=%v", issuerAddress, transferID, r)
			}
		}()
		_, _, errMsg := s.server.MakeRemoteRequestJSON(issuerAddress, "/exchange/rollback", http.MethodPost, map[string]any{
			"token_id":    tokenID,
...
func (s *Server) enforceSelectorDecisionDeadline(transferID, sender, receiver string, deadline float64) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC in enforceSelectorDecisionDeadline transfer=%s sender=%s receiver=%s err=%v", transferID, sender, receiver, r)
		}
	}()
	waitSec := deadline - nowSec()
	if waitSec > 0 {
...
func (s *Server) listRemoteServersByReputation() []string {
	...
	addEntry := func(addr, serverID string, latency float64) {
		addr = trim(addr)
		if addr == "" || core.MessageServerAddressesEqual(addr, s.server.Address, s.server.BindAddress) {
			return
		}
		if serverID != "" {
			if serverID == s.server.ServerID {
				return
			}
			if _, exists := seenID[serverID]; exists {
				return
			}
			seenID[serverID] = addr
		}
		for _, e := range entries {
			if core.NormalizeMessageServerAddress(addr) == core.NormalizeMessageServerAddress(e.address) {
				return
			}
		}
		entries = append(entries, serverEntry{address: addr, serverID: serverID, latency: latency})
	}

	rows, err := s.server.DB.Query(`SELECT address, COALESCE(server_id, ''), COALESCE(latency, 0) FROM known_servers WHERE is_active = 1 ORDER BY last_connected DESC`)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var addr, serverID string
			var latency float64
			if rows.Scan(&addr, &serverID, &latency) != nil {
				continue
			}
...
func (s *Server) fetchContentFromNetwork(contentHash string) bool {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("PANIC in requestContentFromClients goroutine hash=%s err=%v", contentHash, r)
				clientResult <- false
			}
		}()
	...
	if len(servers) > 0 {
		...
		for _, serverAddr := range servers {
			...
			go func() {
				defer func() {
					if r := recover(); r != nil {
						log.Printf("PANIC in fetchContentFromNetwork goroutine addr=%s hash=%s err=%v", addr, contentHash, r)
					}
				}()
				okMeta, payload, _ := s.server.MakeRemoteRequestJSON(addr, "/sync/content?content_hash="+url.QueryEscape(contentHash), http.MethodGet, nil)
				items := []map[string]any{}
				if okMeta {
					items = castSliceMap(payload["items"])
				}
				okRaw, raw, _ := s.server.MakeRemoteRequestBytes(addr, "/content/"+url.PathEscape(contentHash)+"?recursion_guard=1", http.MethodGet)
				if okRaw && len(raw) > 0 && len(items) > 0 {
					select {
					case results <- serverResult{serverAddr: addr, items: items, raw: raw}:
					default:
					}
				} else {
					select {
					case results <- serverResult{serverAddr: addr}:
					default:
					}
				}
			}()
		}
		var firstResult *serverResult
		for i := 0; i < len(servers); i++ {
			select {
			case r := <-results:
				if r.raw != nil && len(r.items) > 0 {
					firstResult = &r
					cancel()
					goto processContentResult
				}
			case <-ctx.Done():
				cancel()
				goto processContentResult
	...
	processContentResult:
		if firstResult != nil {
			serverAddr := firstResult.serverAddr
			items := firstResult.items
			raw := firstResult.raw
			log.Printf("content propagation: found hash=%s from server=%s", contentHash, serverAddr)
			filePath := s.server.ContentPath(contentHash)
...
func (s *Server) resolveDNSFromNetwork(domain string) bool {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("PANIC in requestDDNSFromClients goroutine domain=%s err=%v", domain, r)
				clientResult <- false
			}
		}()
		clientResult <- s.requestDDNSFromClients(domain)
	}()
	select {
	case ok := <-clientResult:
		if ok {
			log.Printf("dns propagation: client supplied domain=%s", domain)
			return true
		}
		clientResult = nil
	...
	results := make(chan dnsResult, len(servers))
	for _, serverAddr := range servers {
		addr := serverAddr
		go func() {
			defer func() {
				if r := recover(); r != nil {
					log.Printf("PANIC in resolveDNSFromNetwork goroutine addr=%s domain=%s err=%v", addr, domain, r)
				}
			}()
			...
			if valid {
				...
				select {
				case results <- dnsResult{serverAddr: addr}:
				default:
				}
			}
		}()
	...
	for i := 0; i < len(servers); i++ {
		select {
		case r := <-results:
			if r.dnsData != nil && asString(r.dnsData["content_hash"]) != "" {
				lastData = r.dnsData
				lastAddr = r.serverAddr
				cancel()
				goto processResult
			}
		case <-ctx.Done():
			log.Printf("resolveDNSFromNetwork: deadline exceeded domain=%s servers=%d", domain, len(servers))
			if lastData == nil {
				if clientResult != nil {
					return <-clientResult
				}
...
processResult:
	contentHash := asString(lastData["content_hash"])
	ddnsHash := asString(lastData["ddns_hash"])
	ddnsPath := ""
	if ddnsHash != "" {
		ddnsPath = s.server.DdnsPath(ddnsHash)
	}
	if ddnsHash == "" || !fileExists(ddnsPath) {
		okDDNS, ddnsBytes, _ := s.server.MakeRemoteRequestBytes(lastAddr, "/ddns/"+url.PathEscape(domain), http.MethodGet, ctx)
	...
	_, _ = s.server.DB.Exec(`INSERT OR REPLACE INTO dns_records
		...
		domain,
		contentHash,
		asString(lastData["username"]),
		defaultStr(asString(lastData["original_owner"]), asString(lastData["username"])),
		asFloat(lastData["timestamp"]),
		asString(lastData["signature"]),
		intFromBool(asBool(lastData["verified"])),
		nowSec(),
		ddnsHash,
		defaultStr(asString(lastData["issuer_server"]), lastAddr),
		asString(lastData["issuer_public_key"]),
		asString(lastData["issuer_contract_id"]),
		asFloat(lastData["issuer_issued_at"]),
	)
	if contracts := castSliceMap(lastData["contracts"]); len(contracts) > 0 {
		items := make([]any, 0, len(contracts))
		for _, contract := range contracts {
			items = append(items, contract)
		}
		s.server.UpsertContractsFromSyncPayload(lastAddr, map[string]any{"items": items})
	}
	return true
}
...
func (s *Server) requestDDNSFromClients(domain string) bool {
	...
	if len(clientIDs) == 0 {
		clientIDs = s.authenticatedClientIdentifiers()
		if len(clientIDs) == 0 {
			log.Printf("dns propagation: no client candidates domain=%s", domain)
			return false
		}
		log.Printf("dns propagation: no indexed candidates domain=%s; broadcasting to authenticated clients=%d", domain, len(clientIDs))
	}
...
func (s *Server) requestContractFromClients(contractID string) bool {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC in requestContractFromClients contract=%s err=%v", contractID, r)
		}
	}()
	contractID = trim(contractID)
	if contractID == "" {
...
func (s *Server) requestContractsForDomainFromClients(domain string) bool {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC in requestContractsForDomainFromClients domain=%s err=%v", domain, r)
		}
	}()
	domain = strings.ToLower(trim(domain))
	if domain == "" {
